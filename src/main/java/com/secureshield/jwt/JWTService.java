...
public class JWTService {
    ...
    public String refreshAccessToken(String refreshToken) {
        ...
        validateForType(claims, REFRESH);

        String tokenId = claims.getId();
        if (!usedRefreshTokenIds.add(tokenId)) {
    ...
    public boolean verifyAccessToken(String token) {
        try {
            Claims claims = parseSignedClaims(token);
            return !tokenValidator.isExpired(claims)
                    && !revokedTokenIds.contains(claims.getId())
                    && tokenValidator.hasExpectedType(claims, ACCESS);
        } catch (Exception ex) {
            return false;
        }
    }
    ...
    public boolean verifyRefreshToken(String token) {
        try {
            ...
            return !tokenValidator.isExpired(claims)
                    && !revokedTokenIds.contains(claims.getId())
                    && tokenValidator.hasExpectedType(claims, REFRESH);
        } catch (Exception ex) {
            return false;
        }
    }
    ...
    private void validateForType(Claims claims, String expectedType) {
        if (!tokenValidator.hasExpectedType(claims, expectedType)) {
            throw new InvalidTokenException("Unexpected token type");
        }
        if (tokenValidator.isExpired(claims)) {
